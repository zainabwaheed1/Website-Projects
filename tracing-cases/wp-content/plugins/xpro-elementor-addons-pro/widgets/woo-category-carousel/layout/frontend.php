...
if ( $terms ) {
	foreach ( $terms as $term ) :
		...
		?>
		<div id="xpro-woo-product-grid-id-<?php echo esc_attr( $term_id ); ?>" class=" xpro-woo-product-grid-item">
			<?php
			//make full div clickable
			if ( 'yes' === $settings['clickable_div'] ) {
				?>
			...
			<div class="xpro-woo-product-grid-img">
				<div class="xpro-woo-product-img-section">
					...
					if ( ( ! empty( $image_id ) ) && 0 !== $image_id && $image_id > 0 ) {
						...
						$placeholder_url = esc_url( Utils::get_placeholder_image_src() );
						$img_src         = $placeholder_url;
					}
					?>
					<img class="xpro-woo-product-grid-img xpro-product-img-url" src="<?php echo esc_url( $img_src ); ?>" alt="<?php echo esc_attr( $term->name ); ?>">
				</div>
			</div>
			<!-- image wrapper end -->
			...
			<div class="xpro-woo-product-grid-content-sec">
				<div class="xpro-woo-product-grid-inner-content-sec">
					...
					if ( 'yes' === $settings['show_content'] ) {
						?>
						<!-- content -->
						<p class="xpro-woo-product-grid-excerpt"><?php xpro_elementor_kses( $content ); ?></p>
						<!-- content end -->
					<?php } ?>
	...
	<p class="xpro-alert xpro-alert-warning">
		<span class="xpro-alert-title"><?php echo esc_html__( 'No Category Found!', 'xpro-elementor-addons-pro' ); ?></span>
