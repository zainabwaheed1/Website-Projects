...
class UniteCreatorWebAPIWork{
	...
	private function modifyArrData_addonTags($addon){
		...
		$strTags = UniteFunctionsUC::getVal($addon, "tags");
	...
	public function getCatalog_blog(){
		...
		$arrCatalogBlog = UniteFunctionsUC::getVal($arrCatalog, "blog");
	...
	public function getCatalogAddonsByTags($tag, $isBG = false){
		...
		foreach($arrAddons as $addon){
			...
			$tagValue = UniteFunctionsUC::getVal($addon, $tag);
	...
	protected function callAPI($action, $body = array(), $isRawResponse = false){
		...
		// collect data
		$body["action"] = $action;
		$body["domain"] = GlobalsUC::$current_host;
		...
		$success = UniteFunctionsUC::getVal($data, "success");
		$success = UniteFunctionsUC::strToBool($success);

		if($success === false){
			$message = UniteFunctionsUC::getVal($data, "message");

			if(empty($message) === true)
				$message = "Something went wrong.";

			UniteFunctionsUC::throwError("Server Error: $message");
		}
